...
object CellVariantcaller extends ToolCommand[Args] {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val regions = sc.parallelize((cmdArgs.intervals match {
      case Some(file) =>
        BedRecordList
          .fromFile(file)
          .combineOverlap
    ...
    val variants = regions.mapPartitions { it =>
      it.flatMap { x =>
        x.flatMap { region =>
...
//      .filter(r =>
//        r.getReadMapped && (cmdArgs.umiTag.isDefined || !r.getDuplicateRead))
//      .flatMap { read =>
...
//        sample.toList
//          .flatMap { s =>
//            val umni = cmdArgs.umiTag.flatMap { u =>
//              attributes
//                .find(_.startsWith(u + ":"))
//                .map(_.split(":")(2))
